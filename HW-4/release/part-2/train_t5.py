...
def clean_decoded_query(query):
    ...
    # Remove AND 1=1 tautologies (ATIS dataset artifact)
    ...
    query = re.sub(r'\bWHERE\s+1\s*=\s*1\b', '', query, flags=re.IGNORECASE)

    # Fix number stuck to closing paren
...
def train(args, model, train_loader, dev_loader, optimizer, scheduler):
    ...
    # Save outputs under per-experiment subfolders for easier organization.
...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    error_rate = np.mean([bool(error_msg) for error_msg in error_msgs]) if error_msgs else 0.0
...
def main():
    ...
    # Test set
    model_sql_path = os.path.join(results_dir, f't5_{model_type}_{experiment_name}_test.sql')
    model_record_path = os.path.join(records_dir, f't5_{model_type}_{experiment_name}_test.pkl')
